...
foreach ($grades as $gradek => $gradev) {
    $acum_m += $gradev['scores']['mat'];
    $cont_m++;
}

$promedio = number_format(($acum_m/$cont_m),2,'.',',');

print("Promedio notas de todos los grupos en matematicas: {$promedio}\n");
print("\n");
...
foreach ($grades as $gradek => $gradev) {
    ...
    $student_average = number_format(($acum_grades/count($gradev['scores'])), 2, '.',',');

    if ($student_average > $previous_average) {

        $top_student = $gradev['student_name'];
        $previous_average = $student_average;
    }

    echo "<br>";

    print("Promedio notas de {$student_name}: {$student_average}\n");
}
...
print("El pelao más aplicado es {$top_student}: {$previous_average}\n");